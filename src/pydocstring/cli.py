...
@click.option('--to', 'target_style', type=click.Choice(['google', 'sphinx']), required=True,
              help='Target docstring style')
@click.option('--from', 'source_style', type=click.Choice(['google', 'sphinx', 'auto']),
              default='auto', help='Source docstring style (default: auto-detect)')
@click.option('--dry-run', is_flag=True, help="Don't write files, just show what would change")
...
@click.option('--include', 'include_globs', multiple=True, metavar='GLOB',
              help='Include files matching glob pattern')
@click.option('--exclude', 'exclude_globs', multiple=True, metavar='GLOB',
...
def convert(
    ...
    if not quiet:
        for file_result in result.file_results:
            rel = file_result.path.relative_to(project_root) if file_result.path.is_relative_to(project_root) else file_result.path

            if file_result.error:
                click.echo(f"ERROR {rel}: {file_result.error}", err=True)
            elif file_result.changed:
                if verbose or diff or dry_run:
                    click.echo(f"{'Would change' if effective_dry_run else 'Changed'}: {rel}")
                if diff:
                    ...
                    diff_lines = difflib.unified_diff(
                        ...
                        fromfile=str(rel), tofile=str(rel) + ' (converted)',
                    )
