...
import pydocstring.strip as strip_tool
...
def test_rewrite_paths_updates_python_files(tmp_path: Path):
    """[Unit] strip_tool: strip_paths rewrites Python files and reports changed count.

    Scenario: Call strip_paths on a directory containing one Python file with a body.
    Boundaries: Single .py file with a return statement; body replaced by pass, docstring kept.
    On failure, first check: strip_paths file discovery, strip_function_bodies, and files_changed count.
    """
    ...
    result = strip_tool.strip_paths([tmp_path])
...
def test_rewrite_paths_can_rename_files_by_pattern(tmp_path: Path):
    """[Unit] strip_tool: strip_paths renames files according to the given pattern.

    Scenario: Call strip_paths with a rename_pattern and start_index on a directory with two files.
    Boundaries: Two .py files; renamed to sequential pattern names; bodies stripped, files_renamed counted.
    On failure, first check: strip_paths rename_pattern substitution and file rename logic.
    """
